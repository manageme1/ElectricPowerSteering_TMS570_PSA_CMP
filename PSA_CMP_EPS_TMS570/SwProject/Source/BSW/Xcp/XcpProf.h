...
#elif defined ( XCP_TRANSPORT_LAYER_TYPE_LIN )
  #define kXcpMaxCTO kLinXcpMaxCTO
  #define kXcpMaxDTO kLinXcpMaxDTO
#endif
...
/*-----------------------------------------------------------------------------------*/
  ...
  #else
    #if defined ( XCP_TRANSPORT_LAYER_TYPE_CAN ) && !defined ( XCP_TRANSPORT_LAYER_TYPE_FLEXRAY )
      #define ApplXcpSend(len, msg) ((XCP_TRANSPORT_LAYER_CAN == xcp.Daq.ActiveTl) ? XcpCanSend(len, msg) : XcpLinSend(len, msg))
      #if defined ( ApplXcpSendFlush )
      #else
        #define ApplXcpSendFlush()
      #endif
      #define ApplXcpInit()          { XcpLinInit(); XcpCanInit(); }
      #define ApplXcpBackground()    { XcpLinBackground(); XcpCanBackground(); }
    #else
      #error "It is not supported to use the Transport Layer for LIN or CAN together with the Transport Layer for FlexRay"
    #endif
  #endif
#endif
...
/* Used only, if write protection of memory areas is required */
  ...
  #else
extern vuint8 ApplXcpCheckWriteAccess( const MTABYTEPTR address, vuint8 size );
  #endif
#endif
...
/* Used only, if read protection of memory areas is required */
  ...
  #else
extern vuint8 ApplXcpCheckReadAccess( const MTABYTEPTR address, vuint32 size );
  #endif
#endif
...
/* Used only, if DAQ protection of memory areas is required */
  ...
  extern vuint8 ApplXcpCheckDAQAccess(DAQBYTEPTR address, vuint8 size);
  #endif
#endif
...
/* Used only, if programming protection of memory areas is required */
  ...
  #else
extern vuint8 ApplXcpCheckProgramAccess( MTABYTEPTR address, vuint32 size );
  #endif
#endif

/* DAQ resume */
#if defined ( XCP_ENABLE_DAQ_RESUME )
extern void ApplXcpDaqResumeStore( const tXcpDaq * daq );
...
extern vuint8 ApplXcpFlashProgram( const BYTEPTR data, MTABYTEPTR address, vuint8 size );
/* Returns:
...
*/
extern vuint8 ApplXcpCheckWriteEEPROM( MTABYTEPTR addr, vuint8 size, const BYTEPTR data );
#endif
...
*/
extern vuint8 ApplXcpTLService( const BYTEPTR pCmd );
#endif
...
/* Return values for ApplXcpOpenCmdIf:
 ...
 XCP_CMD_UNKNOWN      - not implemented optional command
*/
extern vuint8 ApplXcpOpenCmdIf( const BYTEPTR pCmd, BYTEPTR pRes, BYTEPTR pLength );
#endif
...
/* Check consistency of time stamp */

#if defined ( XCP_ENABLE_DAQ_TIMESTAMP ) && defined ( XCP_DISABLE_DAQ_TIMESTAMP )
  #error "XCP consistency error: Time stamp must be either enabled or disabled."
#endif
#if defined ( XCP_ENABLE_DAQ_TIMESTAMP ) || defined ( XCP_DISABLE_DAQ_TIMESTAMP )
#else
  #error "XCP consistency error: Time stamp must be enabled or disabled."
#endif
#if defined ( XCP_ENABLE_DAQ_TIMESTAMP ) && defined ( XCP_DISABLE_DAQ )
  #error "XCP consistency error: Time stamp cannot be used without DAQ."
#endif
...
/* Check if option MODIFY_BITS is valid */
    ...
    #error "XCP error: MODIFY_BITS not supported when address doubling is enabled."
  #endif
#endif
