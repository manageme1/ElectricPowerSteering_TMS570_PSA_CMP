...
static vuint8 XcpWriteMta( vuint8 size, const BYTEPTR data )
{
  ...
  {
#if defined ( XCP_ENABLE_ADDRESS_DOUBLING_SUPPORT )
    if(1 == ((vuint32)xcp.Mta & (vuint32)0x01) )
    {
      XCP_WRITE_BYTE_2_ADDR( (MTABYTEPTR)((vuint32)xcp.Mta >> 1),
         (XCP_READ_BYTE_FROM_ADDR( (MTABYTEPTR)((vuint32)xcp.Mta >> 1) ) & 0x00FF) | (*data << 8) );
    }
    else
    {
      XCP_WRITE_BYTE_2_ADDR( (MTABYTEPTR)((vuint32)xcp.Mta >> 1),
         (XCP_READ_BYTE_FROM_ADDR( (MTABYTEPTR)((vuint32)xcp.Mta >> 1) ) & 0xFF00) | (*data & 0xFF) );
    }
#else
...
static vuint8 XcpReadMta( vuint8 size, BYTEPTR data )
{
  ...
  r = ApplXcpCheckReadAccess(xcp.Mta,(vuint32)size);
  if ( r == (vuint8)XCP_CMD_DENIED )
  {
    xcp.Mta += size;
    return (vuint8)XCP_CMD_DENIED;
  ...
  {
#if defined ( XCP_ENABLE_ADDRESS_DOUBLING_SUPPORT )
    if( 1 == xcp.bAddressDoubled )
    {
      if(1 == ((vuint32)xcp.Mta & (vuint32)0x01) )
      {
        *(data) = (XCP_READ_BYTE_FROM_ADDR( (MTABYTEPTR)((vuint32)xcp.Mta >> 1) ) >> 8);
      }
      else
      {
        *(data) = (XCP_READ_BYTE_FROM_ADDR( (MTABYTEPTR)((vuint32)xcp.Mta >> 1) ) & 0xFF);
      }
    }
    else
    {
      *(data) = XCP_READ_BYTE_FROM_ADDR( xcp.Mta ) & 0xFF;
    }
#else
    /* 
        ...
        else
        {
          ...
          *d = (vuint8)0xFFu; /* Invalidate STIM data buffer */
        ...
        #if defined ( XCP_ENABLE_ADDRESS_DOUBLING_SUPPORT )
          {
            ...
            while(n > 0)
            {
              if(1 == ((vuint32)dest & (vuint32)0x01) )
              {
                XCP_WRITE_BYTE_2_ADDR( (DAQBYTEPTR)((vuint32)dest >> 1),
                  (XCP_READ_BYTE_FROM_ADDR( (DAQBYTEPTR)((vuint32)dest >> 1) ) & 0x00FF) | (*(d+1) << 8));
              }
              else
              {
                XCP_WRITE_BYTE_2_ADDR( (DAQBYTEPTR)((vuint32)dest >> 1),
                  (XCP_READ_BYTE_FROM_ADDR( (DAQBYTEPTR)((vuint32)dest >> 1) ) & 0xFF00) | *(d+1));
              }
              dest++;
              d++;
              n--;
      ...
      for (odt=DaqListFirstOdt(daq);odt<=DaqListLastOdt(daq);odt++)
      {
        ...
        while (e<=el)
        {
      ...
      #if defined ( XCP_ENABLE_ADDRESS_DOUBLING_SUPPORT )
          {
            ...
            while(n > 0)
            {
              if(1 == ((vuint32)src & (vuint32)0x01) )
              {
                *(d) = (XCP_READ_BYTE_FROM_ADDR( (DAQBYTEPTR)((vuint32)src >> 1) ) >> 8);
              }
              else
              {
                *(d) = (XCP_READ_BYTE_FROM_ADDR( (DAQBYTEPTR)((vuint32)src >> 1) ) & 0xFF);
              }
              src++;
              d++;
              n--;
...
/* Table for CCITT checksum calculation */
  ...
  V_MEMROM0 const vuint16 CRC16CCITTtab[256] = {
...
vuint8 XcpBackground( void )
{
  ...
  #endif
    xcp.SessionStatus &= (SessionStatusType)(~SS_STORE_DAQ_REQ & 0xFFu);
  ...
  #endif
    xcp.SessionStatus &= (SessionStatusType)(~SS_CLEAR_DAQ_REQ & 0xFFu);
    /* #### Send an event message */
  }
  if ( (xcp.SessionStatus & (SessionStatusType)SS_STORE_CAL_REQ) != 0 )
  {
    ...
    if ( ret != 0 )
    {
      ...
      xcp.SessionStatus &= (SessionStatusType)(~SS_STORE_CAL_REQ & 0xFFu);
      /* #### Send an event message */
    }
  }
  #endif
  ...
  */
  ...
  if ( (xcp.CheckSumSize) != (tXcpChecksumSumType)0u )
  {
    ...
    do
    {
      ...
      #if defined ( XCP_ENABLE_ADDRESS_DOUBLING_SUPPORT )
        data = XCP_READ_CHECKSUMVALUE_FROM_ADDR( (MTABYTEPTR)((vuint32)xcp.Mta >> 1) );
        #if ( 1 == tXcpChecksumAddSize )
          if(1 == ((vuint32)xcp.Mta & (vuint32)0x01) )
          {
            data = (data >> 8);
          }
      ...
      xcp.CheckSum += (tXcpChecksumSumType)(data);
        ...
        xcp.CheckSum &= 0xFF;
      #endif
      xcp.Mta      += (vuint8)tXcpChecksumAddSize;
    ...
    CRM_BUILD_CHECKSUM_RESULT_WRITE(xcp.CheckSum);
  ...
  else /* CC_CONNECT */
  {
    if ( (xcp.SessionStatus & (SessionStatusType)SS_CONNECTED) != 0 )
    {
          ...
          case CC_GET_ID:
            {
  ...
  #endif
              {
                ...
                MTABYTEPTR pData;
                CRM_GET_ID_LENGTH_WRITE(ApplXcpGetIdData(&pData, CRO_GET_ID_TYPE));
          ...
          case CC_GET_STATUS: 
            {
              ...
              CRM_GET_STATUS_CONFIG_ID_WRITE(xcp.Daq.DaqConfigId);
#else
          ...
          case CC_MODIFY_BITS:
            {
              ...
              data = ((data) & ((~((vuint32)(((vuint32)(~(vuint32)CRO_MODIFY_BITS_AND))<<CRO_MODIFY_BITS_SHIFT))))
                                           ^((vuint32)(((vuint32)CRO_MODIFY_BITS_XOR)<<CRO_MODIFY_BITS_SHIFT)));

              err = XcpWriteMta(sizeof(data), (const BYTEPTR)&data);
              xcp.Mta = Mta_backup;
              if (err==(vuint8)XCP_CMD_PENDING) 
              {
                goto no_response;
      ...
      ( kXcpChecksumMethod == XCP_CHECKSUM_TYPE_CRC32 )
              ...
              {
                CRM_BUILD_CHECKSUM_RESULT_WRITE(0xFFFFu); /* Range, max. 64K-1 */
        ...
        ( kXcpChecksumMethod == XCP_CHECKSUM_TYPE_ADD44 )        
                ...
                {
  #if defined ( XCP_ENABLE_READ_PROTECTION )
                  if( (vuint8)XCP_CMD_DENIED == ApplXcpCheckReadAccess(xcp.Mta, s) )
                  {
                    xcp.Mta += s;
                    CRM_BUILD_CHECKSUM_RESULT_WRITE(0xFFFFu); /* Range, max. 64K-1 */
          ...
          case CC_GET_SECTOR_INFO:
            {
              ...
              {
                CRM_PROGRAM_GET_SECTOR_SECTOR_INFO_WRITE(kXcpProgramSectorStart[sector]);
              ...
              {
                CRM_PROGRAM_GET_SECTOR_SECTOR_INFO_WRITE((kXcpProgramSectorEnd[sector]-kXcpProgramSectorStart[sector])+(vuint32)1u);
    ...
    #if defined ( XCP_ENABLE_PARAMETER_CHECK )
                  if ( ((vuint8)CRO_WRITE_DAQ_MULTIPLE_SIZE(NoDAQ)==(vuint8)0u )
                     || (CRO_WRITE_DAQ_MULTIPLE_SIZE(NoDAQ) > (vuint8)XCP_MAX_ODT_ENTRY_SIZE)
...
vuint8 XcpSendCallBack( void )
{
  ...
  if ( (xcp.SessionStatus & (SessionStatusType)SS_BLOCK_UPLOAD) != 0 )
  {
    ...
    xcp.SessionStatus &= (SessionStatusType)(~SS_BLOCK_UPLOAD & 0xFFu);
    XcpCommand( &cmd.dw );
    END_PROFILE(2); /* Timingtest */
