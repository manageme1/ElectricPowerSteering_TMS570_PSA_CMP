...
/* -- Logical Marker -- */
# define LM_Upload         (vuint8)0x80u
# define LM_Calibration    (vuint8)0x81u
# define LM_Global         (vuint8)0x82u
# define LM_LogZone        (vuint8)0x83u
# define LM_UploadNoCrc    (vuint8)0x84u
# define LM_AppliKey       (vuint8)0x88u
...
typedef struct tagFlashEraseSection
{
   vuint8          sectorNo;
   vuint8          blockType;           /**< Codeblock, datablock, SWM-Data */
   FBL_ADDR_TYPE     address;
   FBL_MEMSIZE_TYPE  length;
} tFlashEraseSection;

typedef struct tagFlashEraseTable
{
   vuint8    magicFlag;          /**< Value of FBL_FLASH_ERASE_TABLE_EXIST_CODE */
                                 /* Indicates the existence of the table */
   vuint8    noOfSections;       /**< MUST correspond to the number of entries of the next table */

   /** Table of erase sections to be erased with service B1 00 B2 xx (Sector erase) */
   tFlashEraseSection FlashEraseSection[FBL_MTAB_NO_OF_SECTIONS];
} tFlashEraseTable;

V_MEMROM0 extern V_MEMROM1 tFlashEraseTable V_MEMROM2 FlashEraseTable;

/***********************************************************************************************************************
 *  PROTOTYPES
 **********************************************************************************************************************/

extern void FblStart(tCanInitTable *pCanInitTable);
