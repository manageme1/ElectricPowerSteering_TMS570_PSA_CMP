...
attrib -r "..\tools\contract\*.h" >>logfile.txt
 ...
 set dcf=Ap_DampingFirewall


set SWC=Ap_DampingFirewall


..\..\Autosar3Tools\RteGen\dvrtegen -d "..\autosar\%dcf%.dcf" -m %SWC% -g i -o "..\src" >>logfile.txt
..\..\Autosar3Tools\RteGen\dvrtegen -d "..\autosar\%dcf%.dcf" -m %SWC% -g c -o "..\tools\contract\Ap_DampingFirewall" >>logfile.txt