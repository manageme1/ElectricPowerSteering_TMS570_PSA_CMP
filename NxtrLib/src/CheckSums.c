/*****************************************************************************
* Copyright 2010 Nexteer, Inc., All Rights Reserved.
* Nexteer Confidential
*
* Module File Name  : CheckSums.c
* Module Description: This file contains the checksum functions
* Product           : Gen II Plus - EA3.0
* Author            : Bobby OSteen
*****************************************************************************/
/*---------------------------------------------------------------------------
* Version Control:
* Date Created:      Mon Feb 04 19:06:38 2010
* %version:          1 %
* %derived_by:       wz7x3j %
* %date_modified:    Fri Feb 11 18:02:02 2011 %
*---------------------------------------------------------------------------*/
#include "CheckSums.h"
#include "fpmtype.h"
#include "fixmath.h"


/* CRC LOOKUP TABLE
* The following CRC lookup table was generated automatically
* using the following model parameters:
* Width : 8 bits
* Polynomial : x^8 + x^4 + x^3 + x^2 + 1
* 0x11d (1-0001-1101)
* [CRC8003]
* Initial Register Value : 0x0
* Reflect Input : false
* * Reflect Output : false
* Value To XOR Output : 0x0
*/
CONST(uint8, CHECKSUM_CONST) CrcTable[256] =
        {};

        
FUNC(uint8, CHECKSUM_CODE) BMW_CRC(	VAR(uint16, AUTOMATIC) id, 
											CONSTP2CONST(uint8, CHECKSUM_CONST, CHECKSUM_CONST) MsgData, 
											VAR(uint8, AUTOMATIC) MsgLength)
{
	VAR(uint8, AUTOMATIC) CheckSum;
	VAR(uint16, AUTOMATIC) kk = 0;
    
   /* Calulate CRC of low byte of ApplicationId  */
   CheckSum = CrcTable[(uint8)(id & 0x00ff)];

   /* Calulate CRC of high byte of ApplicationId */
   CheckSum = CrcTable[CheckSum ^ (uint8)(id >> 8)];


	/*8Bit CRC*/
	for (kk = 0; kk < MsgLength; kk++){
		 CheckSum = CrcTable[CheckSum ^ MsgData[kk]];
	}

	return(CheckSum);
}

/******************************************************************************
CHANGE HISTORY:

 Date      Rev      Author 			Change Description              SCR #
 				 					 (MDD Rev & Date) 		
-------   -------  --------  -----------------------------------  ----------
02/04/11   1.0      OSteen    Initial version 

******************************************************************************/
