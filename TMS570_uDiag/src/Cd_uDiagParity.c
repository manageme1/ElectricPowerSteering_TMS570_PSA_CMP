...
*
* Module File Name  : Cd_uDiagParity.c
* Module Description: Implementation of Micro Diag Parity
* Project           : CBD 
 ...
 * 04/07/14   8       KMC           Added DMA Parity support; corrected HTU parity support                     11435
 * 04/15/16   9       AJM           Corrected Enabling DCAN2 ESM Interrupt and DCAN1 Interrupt priority        EA3#6757
                                    to low
 */
...
#define D_HTUPARITYCFG_CNT_U32			0x0000000AU		/* PARITY ENA Bits 3-0: 5 = Disabled, Other = Enabled */
...
STATIC FUNC(void, SA_AP_MICRODIAG_APPL_CODE) DMAParityInit(void);
...
*              Enables parity check in htu registers and htu parity ESM
*              configuration.  Enable of parity check is also done in htu
*              initialization in some versions of ePWM/Nhet components but
*              is also included here for compatibility with other older components.
* Inputs:      None
* Outputs:     None
*****************************************************************************/
STATIC FUNC(void, SA_AP_MICRODIAG_APPL_CODE) HTUParityInit(void)
{
	...
	/* Ensure Parity Checking is enabled */
	HTU0REG->PCR = D_HTUPARITYCFG_CNT_U32;
#endif

#if (N2HET2TU_PARITY_ENABLE)
	/* Ensure Parity Checking is enabled */
	HTU1REG->PCR = D_HTUPARITYCFG_CNT_U32;
#endif

#if ((N2HET1TU_PARITY_ENABLE == STD_ON) || (N2HET1TU_PARITY_ENABLE == STD_ON))	
	/* Disable the automatic nError Pin response */
...
STATIC FUNC(void, SA_AP_MICRODIAG_APPL_CODE) DCANParityInit(void)
{
	...
	ESMREGS->ESMILCR1 = D_DCAN1PARESMCHANNEL_CNT_U32;
#endif
	...
	/* Enable DCAN ESM Interrupts (Map to ESML) */
	ESMREGS->ESMIESR1 = D_DCAN2PARESMCHANNEL_CNT_U32;
...
/*****************************************************************************
* Name:        DMAParityInit
* Description: DMA Parity Specific Initialization Routine
*              Initializes ESM registers for DMA parity error detection
*              Enable of DMA parity check is initialized in DMA registers
*              in DMA component.
* Inputs:      None
* Outputs:     None
*****************************************************************************/
STATIC FUNC(void, CD_UDIAG_APPL_CODE) DMAParityInit(void)
{
#if (DMA_PARITY_ENABLE)
	/* Disable the automatic nError Pin response */
	ESMREGS->ESMDEPAPR1 = D_DMAPARESMCHANNEL_CNT_U32;

	/* Enable DMA Parity ESM Interrupts (Map to ESML) */
	ESMREGS->ESMIESR1 = D_DMAPARESMCHANNEL_CNT_U32;
	ESMREGS->ESMILCR1 = D_DMAPARESMCHANNEL_CNT_U32;
#endif
}
...
FUNC(void, CD_UDIAG_APPL_CODE) HTUParityErr(void)
{
	...
	if (0U != (HTU0REG->PAR & D_HTUPARERRMASK_CNT_U32))
	{
...
/*****************************************************************************
* Name:        DMAParityErr
* Description: DMA Parity Error Handling
* Inputs:      None
* Outputs:     None
*****************************************************************************/
...
{
#if (DMA_PARITY_ENABLE)
	/* Set Run Time DMA Parity Fault */
	(void)Rte_Call_NxtrDiagMgr_SetNTCStatus(NTC_Num_PerDiag_DMAFlt, D_NTCPARMBIT1_CNT_U8, NTC_STATUS_FAILED);
#endif
}
