 ...
 *  Generated at:  Tue May 14 08:48:27 2013
 *
 *     Generator:  MICROSAR RTE Generator Version 2.19.1
 *       License:  License CBD1010124 valid for CBD1010124 Nexteer  Package: BMW MSR SIP BAC3.0     Micro: TI TMS570 TMS570LS30306  Compiler: Code Composer Studio 4.9.0
 *
 *   Description:  C-Code implementation template for SW-C <Ap_ReturnFirewall>
 *********************************************************************************************************************/
 ...
 *********************************************************************************************************************/
/* Version Control:
 * Date Created:      Mon Apr 27 08:30:00 2012
 * %version:          9 %
 * %derived_by:       lz4p8n %
 * %date_modified:    Tue May 14 10:45:46 2013 %
 *---------------------------------------------------------------------------------------------------------------------
 * Date      Rev     Author    Change Description                                                                SCR #
 * -------   ------  --------  --------------------------------------------------------------------------------  ------
 * 04/27/12  1       OT        Initial Version                                                                   5301
 * 05/10/12  1       OT        Fixed calibration naming conflict                                                 5301
 * 07/30/12  2       NRAR      Added IF block to check if ReturnCmd reaches boundary values                      5905
 * 08/01/12  3       NRAR      Fault is set when ReturnCmd reaches boundaries                                    5905
 * 09/23/12  4        Selva      Checkpoints added and mempmap macros corrected		                             6246
 * 02/01/13  6       VK		   Updates to meet SF-36 v003                                                        7210
 * 02/04/13  7       VK        Corrected compiler warning														 7210
 * 05/13/13  8		 SP		   Implement SF-36 Ver 004															 7821
 * 05/14/13  9		 sp        QAC fixes																		 8269
 */
/**********************************************************************************************************************
...
#include "Ap_ReturnFirewall_Cfg.h"

#define RETURNFIREWALL_START_SEC_VAR_CLEARED_32
#include "MemMap.h" /* PRQA S 5087 */
STATIC volatile VAR(float32, DAMPINGFIREWALL_VAR) UprBound_MtrNm_D_f32;
STATIC volatile VAR(float32, DAMPINGFIREWALL_VAR) LwrBound_MtrNm_D_f32;
#define RETURNFIREWALL_STOP_SEC_VAR_CLEARED_32
#include "MemMap.h" /* PRQA S 5087 */

#define RETURNFIREWALL_START_SEC_VAR_CLEARED_BOOLEAN
#include "MemMap.h" /* PRQA S 5087 */
STATIC volatile VAR(boolean, DAMPINGFIREWALL_VAR) OverBound_Cnt_D_lgc;
#define RETURNFIREWALL_STOP_SEC_VAR_CLEARED_BOOLEAN
#include "MemMap.h" /* PRQA S 5087 */
/**********************************************************************************************************************
 ...
 *********************************************************************************************************************/
	...
	VAR(sint16,  AUTOMATIC) HandwheelPosition_HwDeg_T_s11p4;
	...
	DefeatReturnSvc_Cnt_T_lgc = Rte_IRead_ReturnFirewall_Per1_Defeat_Return_Svc_Cnt_lgc();
	MECCounter_Cnt_T_enum = Rte_IRead_ReturnFirewall_Per1_MEC_Counter_Cnt_enum();
	
	/* Instead of using MEC Counter the enum is being used. enum equals ProductionMode when MEC Counter is 0. */
	if((DefeatReturnSvc_Cnt_T_lgc != D_FALSE_CNT_LGC) && (MECCounter_Cnt_T_enum != ProductionMode)) 
	{
		LimitedReturn_MtrNm_T_f32 = ReturnCmd_MtrNm_T_f32;
		(void) Rte_Call_NxtrDiagMgr_SetNTCStatus(NTC_Num_ReturnFWReached, 0x01U, NTC_STATUS_PASSED);
	}
	else
	{
		UprBound_MtrNm_T_s4p11 = BilinearXYM_s16_s16Xs16YM_Cnt(VehicleSpeed_Kph_T_u9p7,
															   HandwheelPosition_HwDeg_T_s11p4,
															   t_RtrnFWVehSpd_Kph_u9p7,
															   TableSize_m(t_RtrnFWVehSpd_Kph_u9p7),
															   t_RtrnFWUprBoundX_HwDeg_s11p4,
															   (P2CONST(sint16, AUTOMATIC, AP_RETURNFIREWALL_CONST))t2_RtrnFWUprBoundY_MtrNm_s4p11,
															   TableSize_m(t_RtrnFWUprBoundX_HwDeg_s11p4));
		
		UprBound_MtrNm_T_f32 = FPM_FixedToFloat_m(UprBound_MtrNm_T_s4p11, s4p11_T);
		
		LwrBound_MtrNm_T_s4p11 = BilinearXYM_s16_s16Xs16YM_Cnt(VehicleSpeed_Kph_T_u9p7,
															  (D_NEGONE_CNT_S16 * HandwheelPosition_HwDeg_T_s11p4),
															  t_RtrnFWVehSpd_Kph_u9p7,
															  TableSize_m(t_RtrnFWVehSpd_Kph_u9p7),
															  t_RtrnFWUprBoundX_HwDeg_s11p4,
															  (P2CONST(sint16, AUTOMATIC, AP_RETURNFIREWALL_CONST))t2_RtrnFWUprBoundY_MtrNm_s4p11,
		...
		LimitedReturn_MtrNm_T_f32 = Limit_m(ReturnCmd_MtrNm_T_f32, LwrBound_MtrNm_T_f32, UprBound_MtrNm_T_f32);
		...
		if(Abs_f32_m(ReturnCmd_MtrNm_T_f32 - LimitedReturn_MtrNm_T_f32) > FLT_EPSILON)
		{
			/* ReturnCmd signal reaches boundary  */
			OverBound_Cnt_D_lgc = TRUE;
			/*Fault is set when ReturnCmd reaches boundary values */
			(void) Rte_Call_NxtrDiagMgr_SetNTCStatus(NTC_Num_ReturnFWReached, 0x01U, NTC_STATUS_FAILED); 
		}
		else
		{
			OverBound_Cnt_D_lgc = FALSE;
			(void) Rte_Call_NxtrDiagMgr_SetNTCStatus(NTC_Num_ReturnFWReached, 0x01U, NTC_STATUS_PASSED);
