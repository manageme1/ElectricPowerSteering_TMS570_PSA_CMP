...
attrib -r "..\utp\contract\*.h" >>logfile.txt
 ...
 set dcf=Ap_FrqDepDmpnInrtCmp


set SWC=Ap_FrqDepDmpnInrtCmp


..\..\UnitTestEnv\RteGen\dvrtegen -d "..\autosar\%dcf%.dcf" -m %SWC% -g i -o "..\src" >>logfile.txt
..\..\UnitTestEnv\RteGen\dvrtegen -d "..\autosar\%dcf%.dcf" -m %SWC% -g c -o "..\utp\contract\Ap_FrqDepDmpnInrtCmp" >>logfile.txt