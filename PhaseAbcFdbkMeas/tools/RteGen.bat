...
attrib -r "..\tools\contract\*.h" >>logfile.txt
 ...
 set dcf=Cd_PhaseAbcFdbkMeas

set SWC=Cd_PhaseAbcFdbkMeas

..\..\Autosar3Tools\RteGen\dvrtegen -d "..\autosar\%dcf%.dcf" -m %SWC% -g i -o "..\src" >>logfile.txt
..\..\Autosar3Tools\RteGen\dvrtegen -d "..\autosar\%dcf%.dcf" -m %SWC% -g c -o "..\tools\contract\Cd_PhaseAbcFdbkMeas" >>logfile.txt
